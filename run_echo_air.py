    ...
    if check:
        ...
        if run_parallel:
            ...
            exp_pop_alloc_future = executor.submit(
                pop.allocate_population, isrmgrid.geodata, False)
            ...
            exp_pop_alloc = pop.allocate_population(isrmgrid.geodata, False)
        ...
        if run_health:
            ...
            if run_parallel:
                ...
                verboseprint(verbose, '- Health calculation input details in verbose mode will be preceded by the tag [HEALTH].', debug_mode, frameinfo=getframeinfo(currentframe()))
                hia_pop_alloc = pop.allocate_population(isrmgrid.geodata, True)
