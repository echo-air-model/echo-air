        ...
        if run_health:
            ...
            if run_parallel:
                ...
                with concurrent.futures.ProcessPoolExecutor(max_workers=5) as health_executor:
                    ...
                    allcause_ve_future = health_executor.submit(visualize_and_export_hia, allcause, ca_shp_path, 'TOTAL', 'ALL CAUSE', output_resolution, output_dir, f_out, shape_out, verbose=verbose, debug_mode=debug_mode)
                    ihd_ve_future = health_executor.submit(visualize_and_export_hia, ihd, ca_shp_path, 'TOTAL', 'ISCHEMIC HEART DISEASE', output_resolution, output_dir, f_out, shape_out, verbose=verbose, debug_mode=debug_mode)
                    lungcancer_ve_future = health_executor.submit(visualize_and_export_hia, lungcancer, ca_shp_path, 'TOTAL', 'LUNG CANCER', output_resolution, output_dir, f_out, shape_out, verbose=verbose, debug_mode=debug_mode)
                ...
                # Plot and export
                logging.info('<< Exporting Health Impact Outputs >>')
                visualize_and_export_hia(allcause, ca_shp_path, 'TOTAL', 'ALL CAUSE', output_resolution, output_dir, f_out, shape_out, verbose=verbose, debug_mode=debug_mode)
                visualize_and_export_hia(ihd, ca_shp_path, 'TOTAL', 'ISCHEMIC HEART DISEASE', output_resolution, output_dir, f_out, shape_out, verbose=verbose, debug_mode=debug_mode)
                visualize_and_export_hia(lungcancer, ca_shp_path, 'TOTAL', 'LUNG CANCER', output_resolution, output_dir, f_out, shape_out, verbose=verbose, debug_mode=debug_mode)
