...
class health_data:
    ...
    def combine_pop_inc(self):
        ...
        hia_pop, incidence = self.population.copy(), self.incidence.copy()
        
        # Update both dataframes
        hia_pop = self.update_pop(hia_pop)
        ...
        pop_geo = hia_pop[['ISRM_ID','geometry']].drop_duplicates()
        ...
        pop_inc = pd.merge(pop_inc, hia_pop[['ISRM_ID','START_AGE', 'END_AGE', 
                                             'RACE', 'POPULATION']],
                           on='ISRM_ID')
        ...
        lookups = self.incidence_by_age(incidence, hia_pop)
        lookup_dict = self.make_incidence_lookup(incidence, keys, lookups)
        
        # Map values from the lookup dict
        ...
        pop_inc = pop_inc[['ISRM_ID', 'NAME', 'RACE', 'POPULATION','ALL CAUSE INC', 
                           'ISCHEMIC HEART DISEASE INC','LUNG CANCER INC','geometry']].copy()
